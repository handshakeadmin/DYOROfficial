...
const preToolUse: PreToolUseHandler = async (payload) => {
  ...
  // Log when Claude is about to edit files
  if (payload.tool_name === 'Edit' && payload.tool_input) {
    ...
    if (sensitivePatterns.some((p) => file_path.toLowerCase().includes(p))) {
      console.error('❌ Attempting to edit sensitive file! Blocking.')
      return {
        permissionDecision: 'deny',
        permissionDecisionReason: `Cannot edit sensitive file: ${file_path}`,
      }
    }
  }
  ...
  if (payload.tool_name === 'Write' && payload.tool_input) {
    const {file_path} = payload.tool_input as {file_path: string}
    if (file_path.includes('.env') && !file_path.includes('.example')) {
      console.error('❌ Cannot write to .env files!')
      return {
        permissionDecision: 'deny',
        permissionDecisionReason: `Cannot write to environment file: ${file_path}`,
      }
    }
  }
